...
make_volcano <- function(res_df, title, lfc_cut = 1, pval_cut = 0.05) {
  df <- res_df %>%
    filter(!is.na(logFC)) %>%
    mutate(
  ...
  df$label <- if ("Gene_Name" %in% names(df)) {
    ifelse(is.na(df$Gene_Name) | df$Gene_Name == "", df$Gene_ID, df$Gene_Name)
  } else {
    df$Gene_ID
  }
