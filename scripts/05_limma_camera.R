...
# Contrasts: Dark vs FL, LL vs FL, ML vs FL, HL vs FL
...
contrast_matrix <- makeContrasts(
  DarkvsFL = Dark_T30 - FL_T30,
  LLvsFL   = LL_T30 - FL_T30,
  MLvsFL   = ML_T30 - FL_T30,
  HLvsFL   = HL_T30 - FL_T30,
  levels   = design
)
...
contrast_defs <- list(
  DarkvsFL = list(coef = "DarkvsFL", label = "Dark vs FL", axis_label = "Dark - FL"),
  LLvsFL   = list(coef = "LLvsFL",   label = "LL vs FL",   axis_label = "LL - FL"),
  MLvsFL   = list(coef = "MLvsFL",   label = "ML vs FL",   axis_label = "ML - FL"),
  HLvsFL   = list(coef = "HLvsFL",   label = "HL vs FL",   axis_label = "HL - FL")
)

# Contrasts (and their x-axis order) used in the combined multi-contrast
# GO dotplot built in Part C below.
combined_plot_contrasts <- c("DarkvsFL", "LLvsFL", "MLvsFL", "HLvsFL")
...
run_contrast_analysis <- function(contrast_name, cdef) {
  ...
  coef_name      <- cdef$coef
  contrast_label <- cdef$label     # e.g. "HL vs FL"
  axis_label     <- cdef$axis_label # e.g. "HL - FL"
  tag            <- contrast_name  # e.g. "DarkvsFL" -- used in filenames
...
# x = contrast (DarkvsFL, LLvsFL, MLvsFL, HLvsFL), y = GO term,
# colour = Direction, size = -log10(FDR)
...
for (onto in ontologies) {
  ...
  out_file <- file.path(dir_combined, sprintf("camera_GO_dotplot_combined_%s_vsFL.png", onto))
  ggsave(
    filename  = out_file,
    plot      = result$plot,
    width     = 9,
...
if (length(bp_combined_sig_ids) == 0) {
  ...
  if (is.null(result_bp_general)) {
    ...
    out_file_bp_general <- file.path(dir_combined, "camera_GO_dotplot_combined_BP_most_general_vsFL.png")
    ggsave(
